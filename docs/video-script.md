...
Show ledger summary: ~**$87/mo** detected, buckets empty before scan.
...
1. Consent → **Demo Scan**  
2. Land on **`/opportunities`** — 8+ findings  
3. Call out **three different services** (EC2, EBS, RDS)
...
> *"279 Playwright tests including the full operator journey; 420 backend tests; Cedar default deny."*
...
- **SLA replay:** `curl -X POST …/api/replay/api-gateway-sla` — full 11-node graph, ~$0.35  
- **EC2 stop:** `POST /api/ec2-demo/trigger` — live bounded action  

Do **not** open `/replay` unless you explicitly want graph/SSE B-roll; it is not part of J-FULL.
