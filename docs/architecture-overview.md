...
**Test coverage:** 28 Playwright specs · 279 tests · 420 backend tests collected
...
**Primary operator path (J-FULL):** nine AWS account scanners → promote finding → claim-bound approve / investigate / decline → ledger (+ SNS on approve). Documented in [operator-journey.md](operator-journey.md). The **11-node Strands graph** is optional on promote; it remains central to **SLA verified replay** (optional `/replay` / API path).
...
│     (+ /opportunities/[id] HITL; /replay deep link optional)     │
└────────────────────────┬────────────────────────────────────────┘
                         │ HTTP / SSE
┌────────────────────────▼────────────────────────────────────────┐
...
| **Scanner-first UI** | Primary demo is J-FULL account scan → HITL → ledger; Verified Replay remains for graph/SSE depth |
...
Synthetic API Gateway SLA breach
  ...
  → 10% credit tier
  → real billed charges × 10% = credit (e.g. $3.51 × 10% = $0.35 for demo account)
  → Human approves on `/opportunities/{id}`
  → Replay case ID returned (real submission gated by recoup_enable_real_support_submission)
```

Expected result: **consistent positive credit on every run** (amount from real billing_snapshot.json — $0.35 for demo account), P95 ≤ 60 s.

### Proof 2 — Live AWS Action (Phase 6 — Complete)

```
Real EC2 t3.micro (RecoupDemo=true tag)
  → CloudWatch confirms idle CPU (< 1% over 7 days)
  → CloudTrail ownership check
  → Cedar policy evaluates REQUIRE_APPROVAL (5 preconditions)
  → Human approves on `/opportunities/{id}` (LIVE AWS ACTION badge)
  → Real StopInstances call + instance_stopped waiter
...
| EC2 `StopInstances` | Live demo action | ✅ Live |
...
| EventBridge + SQS poller | Auto-trigger replay on Health event | ✅ Live |
...
│   ├── adapters/        replay.py · agentcore.py · ec2_demo.py
│   ├── agents/          strands_agents.py (4 Strands agents backed by Amazon Bedrock)
│   ├── api/             FastAPI: main.py + routes/
│   │                      opportunities · approvals · replay · ec2_demo · quality
│   │                      cloudtrail_demo · cost_demo · tagging_demo · scan
...
├── backend/tests/       420 tests collected, 0 DeprecationWarnings (unit + trajectory + e2e + adversarial)
├── frontend/src/app/    Next.js 16 · routes: / → /opportunities · /scan · /recovery · /replay
