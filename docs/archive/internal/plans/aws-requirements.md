...
| [R2] AgentCore | Runtime + Gateway + Policy/Observability deployed | 🟡 | Harness + Gateway registered (`recoup_recovery_agent-T9RRFljZUO`, `recoup-tool-gateway-tpnzqdgixc`). Strands graph uses `BedrockModel` when `LLM_PROVIDER=bedrock`; `AgentCoreAdapter` for harness/gateway. **Ops verify:** `./scripts/register_agentcore.py --dry-run` + console harness health. |
| [R6] AgentCore Policy | Cedar policies attached in enforcement mode; default deny | ✅ | Fail-closed Cedar in `safety/cedar.py`; claim binding → 409 (**SEC-1/SEC-2/SEC-3**). Gateway: `./scripts/verify_infra.sh` after deploy. |
| [R7] AgentCore Observability | All metrics/logs routed to CloudWatch | ✅ | Log groups wired; `X-Request-ID` (**SEC-5/SEC-6**). Custom metrics: `Recoup` scorecard + `Recoup/Graph` / `Recoup/Tools` hooks when live AWS enabled. |
...
| [R11] Cost Anomaly | Anomaly module uses Impact and RootCauses fields | ✅ | `get_cost_anomalies()` calls `ce.get_anomalies()` when live AWS enabled; maps Impact/RootCauses; graceful stub on error or empty account data. |
| [R12] Cost Opt Hub | Recommendations consumed as signals; not replicated | ✅ | `list_cost_optimization_recommendations()` calls `hub.list_recommendations()` when live; graceful stub on error or empty hub data. |
