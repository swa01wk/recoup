...
**HTTP:** `/api/replay/*` **removed** (J-FULL cleanup) — adapter used by **pytest** (`tests/unit/test_replay_phase2.py`, `tests/e2e/test_golden_replay.py`) and **`GET /api/quality/scorecard`**.  
**Optional E2E graph smoke:** `frontend/e2e/workflow-stages.spec.ts` WF-11 (opportunity run path, not replay HTTP).  
**Status:** Deterministic SLA math — 20/20 consecutive runs; P95 = 23 ms
...
**Product demo:** use [operator-journey.md](operator-journey.md) (J-FULL scanner loop). Replay is **technical depth** — not in the three-link sidebar. Bedrock is bypassed (`use_strands=False`) in canonical pytest runs.
