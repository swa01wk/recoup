...
#   run_ec2_stop_decision_agent(state)     → optional; not on J-FULL promote path
...
**Design invariant:** `use_strands` is opt-in on `POST .../run` (`RunRequest.use_strands`). The canonical SLA replay (pytest adapter) always runs with `use_strands=False` to guarantee 20/20 deterministic results regardless of Bedrock availability.
